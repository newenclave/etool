...
namespace etool { namespace queues { namespace condition { namespace traits {
    ...
    template <typename ValueType> struct simple {
        ...
        static void push(queue_type& q, value_type value)
        {
            q.emplace(std::move(value));
        }

        static value_type& front(queue_type& q)
        {
            return q.front();
        }

        static const value_type& front(const queue_type& q)
        {
            return q.front();
        }
        ...
        static typename queue_type::size_type size(const queue_type& q)
        {
            return q.size();
        }
        ...
        static void clear(queue_type& q)
        {
            q.clear();
        }
    };

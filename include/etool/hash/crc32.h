#ifndef ETOOL_HASH_CRC32_H
#define ETOOL_HASH_CRC32_H

#include <cstdint>

namespace etool { namespace hash {
    ...
    struct crc32 {
        ...
        static bool check(const char* data, std::size_t len, const char* res)
        {
            std::uint32_t oldcrc = 0xFFFFFFFF;
            for (std::size_t i = 0; i < len; ++i) {
                oldcrc = update(oldcrc, static_cast<std::uint8_t>(data[i]));
            }
            oldcrc = ~oldcrc;
        ...
        static void get(const char* data, std::size_t len, char* out)
        {
            std::uint32_t oldcrc = 0xFFFFFFFF;
            for (std::size_t i = 0; i < len; ++i) {
                oldcrc = update(oldcrc, static_cast<std::uint8_t>(data[i]));
            ...
            oldcrc = ~oldcrc;
            out[0] = static_cast<char>((oldcrc >> 24) & 0xFF);
            out[1] = static_cast<char>((oldcrc >> 16) & 0xFF);
            out[2] = static_cast<char>((oldcrc >> 8) & 0xFF);
            out[3] = static_cast<char>((oldcrc)&0xFF);
        }

        static std::uint32_t update(std::uint32_t old, std::uint8_t val)
        {
            return get_value(old ^ val) ^ (old >> 8);
        }

    private:
        static std::uint32_t get_value(std::uint32_t id)
        {
            /* poly 0xedb88320 */
            static const std::uint32_t crc_32_table[]

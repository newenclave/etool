...
namespace etool { namespace details {
    ...
    struct byte_hex {
        ...
        static std::uint8_t char2int(char cc)
        {
        ...
        static result to_Hex(const std::string& hex)
        {
        ...
        static result from_hex(const std::string& hex)
        {
            return from_hex(hex.c_str(), hex.size());
        }

        static result from_hex(const char* hex, std::size_t len)
        {
            std::string res;
            if (len % 2 != 0) {

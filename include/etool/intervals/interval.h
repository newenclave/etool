...
namespace etool { namespace intervals {
    ...
    public:
        ...
        interval(domain_type lh, domain_type rh, attributes lf, attributes rf)
            : left_(std::move(lh))
            , right_(std::move(rh))
        {
            attrs_[0] = lf;
        ...
        explicit interval(const domain_type& val)
            : left_(val)
            , right_(val)
        {
        ...
        bool contains(const domain_type& val) const
        {
            ...
            bool bleft
                = is_minus_inf<EPN::LEFT>() || is_greater_equal<EPN::LEFT>(val);
        ...
        bool empty() const
        {
            return !has_infinite() && has_open() && cmp::equal(left(), right());
        }
        ...
        {
            return interval(left(), to.left(), attrs<endpoint_name::LEFT>(),
    ...
    public: /// factories
        static interval open(domain_type lh, domain_type rh)
        {
            return interval(std::move(lh), std::move(rh), attributes::OPEN,
                            attributes::OPEN);
        }

        static interval closed(domain_type lh, domain_type rh)
        {
            return interval(std::move(lh), std::move(rh), attributes::CLOSE,
                            attributes::CLOSE);
        }

        static interval degenerate(domain_type lh)
        {
            return interval(lh);
        }
        ...
        {
            return interval(domain_type(), domain_type(), attributes::MIN_INF,
                            attributes::MAX_INF);
        }

        static interval minus_infinite()
        {
            return interval(domain_type(), domain_type(), attributes::MIN_INF,
                            attributes::MIN_INF);
        }

        static interval plus_infinite()
        {
            return interval(domain_type(), domain_type(), attributes::MAX_INF,
                            attributes::MAX_INF);
        }

        static interval left_open(domain_type val)
        {
            return interval(std::move(val), domain_type(), attributes::OPEN,
                            attributes::MAX_INF);
        }

        static interval left_open(domain_type lh, domain_type rh)
        {
            return interval(std::move(lh), std::move(rh), attributes::OPEN,
                            attributes::CLOSE);
        }

        static interval right_open(domain_type val)
        {
            return interval(domain_type(), std::move(val), attributes::MIN_INF,
                            attributes::OPEN);
        }

        static interval right_open(domain_type lh, domain_type rh)
        {
            return interval(std::move(lh), std::move(rh), attributes::CLOSE,
                            attributes::OPEN);
        }

        static interval left_closed(domain_type val)
        {
            return interval(std::move(val), domain_type(), attributes::CLOSE,
                            attributes::MAX_INF);
        }

        static interval left_closed(domain_type lh, domain_type rh)
        {
            return right_open(std::move(lh), std::move(rh));
        }

        static interval right_closed(domain_type val)
        {
            return interval(domain_type(), std::move(val), attributes::MIN_INF,
                            attributes::CLOSE);
        }

        static interval right_closed(domain_type lh, domain_type rh)
        {
            return left_open(std::move(lh), std::move(rh));
        }

        static interval intersection(const interval& lh, const interval& rh)
        {
            return interval(lh.left(), rh.right(), lh.left_attr(),
                            rh.right_attr());
        }
        ...
        struct cmp {
            ...
            static bool less(const domain_type& lh, const domain_type& rh)
            {
                return op_cmp::less(lh, rh);
            }

            static bool greater(const domain_type& lh, const domain_type& rh)
            {
                return op_cmp::greater(lh, rh);
            }

            static bool equal(const domain_type& lh, const domain_type& rh)
            {
                return op_cmp::equal(lh, rh);
            }

            static bool equal(const interval& lh, const interval& rh)
            {
                return equal_side<endpoint_name::LEFT>(lh, rh)
                    && equal_side<endpoint_name::RIGHT>(lh, rh);
            }

            static bool less_equal(const domain_type& lh, const domain_type& rh)
            {
                return op_cmp::less_equal(lh, rh);
            }

            static bool greater_equal(const domain_type& lh,
                                      const domain_type& rh)
            {
                return op_cmp::greater_equal(lh, rh);
            }

            static bool less_left(const interval& lh, const interval& rh)
            {
            ...
            static bool equal_side(const interval& lh, const interval& rh)
            {
            ...
            static bool less_right(const interval& lh, const interval& rh)
            {
                using AT = attributes;
            ...
            static bool less(const interval& lh, const interval& rh)
            {
        ...
        private:
            static bool degen(const interval& ival)
            {
            ...
            static bool empty(const interval& ival)
            {
                return ival.empty();
            }

        public:
            static bool equal_empty(const interval& lh, const interval& rh)
            {
    ...
    private:
        ...
        bool is_less_equal(const domain_type& val) const
        {
            ...
            ptr p;
            return is_close<Side>() ? cmp::less_equal(val, value(p))
        ...
        bool is_greater_equal(const domain_type& val) const
        {
            ...
            using ptr = typename V::pointer;
            ptr p;
            return is_close<Side>() ? cmp::greater_equal(val, value(p))
        ...
        bool contains_side(const interval& other) const
        {
            ...
            if (other.is_minus_inf<Side>()) {
                ...
                if (!bleft) {
                    bleft = other.is_open<Side>() && is_open<Side>()
            ...
            if (is_any_inf<Side>() || other.is_any_inf<Op::name>()) {
                ...
                if (eq) {
                    return empty() || other.empty()
                        || (attrs<Side>() == other.connected_attr<Op::name>());
                }
                return false;
            }
        }
    ...
    inline bool operator<(const interval<ValueT, Comparator>& lh,

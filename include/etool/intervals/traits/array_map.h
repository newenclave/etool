...
namespace etool { namespace intervals { namespace traits {
    ...
    struct array_map {
        ...
        struct comparator {
            ...
            bool operator()(const value_type& lh, const value_type& rh) const
            {
                const typename interval_type::cmp_not_overlap cmp;
            ...
            iterator emplace_hint(const_iterator where, value_type val)
            {
                if (where != cend()) {
                    ...
                    if (cless(val, where->first)) {
                        lb = std::lower_bound(cbegin(), where, val.first,
                                              set_cmp());
                    } else {
                        lb = std::lower_bound(where, cend(), val.first,
                                              set_cmp());
                    }
        ...
        struct iterator_access {
            ...
            static const interval_type& key(const_iterator itr)
            {
                return itr->first;
            }

            static interval_type& mutable_key(iterator itr)
            {
                return const_cast<interval_type&>(itr->first);
            }

            static const interval_type& key(const value_type& val)
            {
                return val.first;
            }

            static interval_type& mutable_key(value_type& val)
            {
                return const_cast<interval_type&>(val.first);
            }

            static void copy(value_type& to, const value_type& from)
            {
                to.second = from.second;
                // to = from;
            }

            static const value_type& val(const_iterator itr)
            {
                return *itr;
            }

            static value_type& mutable_val(iterator itr)
            {
                return *itr;
            }
        };

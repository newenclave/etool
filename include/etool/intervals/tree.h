...
namespace etool { namespace intervals {
    ...
    public:
        ...
        iterator find(const key_type& key)
        {
            ...
            if (res.left.itr != cont_.end()) {
                if ((res.left.itr == res.right.itr && res.left.contains
                     && res.right.contains)
        ...
        std::pair<iterator, iterator> find_intersection(const key_type& key)
        {
            using CT = container_type;
        ...
        iterator absorb_impl(value_type ival)
        {
            ...
            if (pair.left.connected) {
                ...
                while (pair.left.itr != cont_.begin()) {
                    pair.left.itr--;
                    if (I::key(pair.left.itr)
                            .right_connected(I::key(first_iter))) {
                        first_iter = pair.left.itr;
                        I::mutable_key(ival).replace_left(I::key(first_iter));
                    } else {
                        break;
            ...
            if (pair.right.contains) {
                I::mutable_key(ival).replace_right(I::key(last_iter));
                last_iter++;
                pair.right.connected
                    = I::key(ival).right_connected(I::key(last_iter));
            }
    ...
    protected:
        ...
        static intersect_pair<ItrT> locate(Cont& cont, const key_type& ival)
        {
#ifdef DEBUG
            if (!ival.valid()) {
        ...
        iterator find_impl(const domain_type& key)
        {
            ...
            return I::key(res.left.itr).contains(key) ? res.left.itr
                                                      : cont_.end();
        }

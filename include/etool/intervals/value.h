...
namespace etool { namespace intervals {
    ...
    public:
        ...
        template <typename... Args>
        static value make(attributes a, Args&&... args)
        {
        ...
        struct cmp {
            ...
            static bool less(const value& lh, const value& rh)
            {
                if (rh.is_max()) {
                    return !lh.is_max();
                } else if (rh.is_min()) {
                ...
                if (lh.is_open()) {
                    return rh.is_open() ? ocmp::less(lh.get(), rh.get())
                                        : ocmp::less_equal(lh.get(), rh.get());
                } else if (lh.is_closed()) {
                    return rh.is_open() ? ocmp::less_equal(lh.get(), rh.get())
                                        : ocmp::less(lh.get(), rh.get());
                }
                return false;
            }

            static bool less(const value& lh, const domain_type& rh)
            {
                switch (lh.attr()) {
            ...
            static bool less(const domain_type& lh, const value& rh)
            {
                switch (rh.attr()) {
                case attributes::MIN_INF:
                    return false;
            ...
            static bool equal(const value& lh, const value& rh)
            {
                switch (lh.attr()) {
                case attributes::MIN_INF:
                case attributes::MAX_INF:
            ...
            static bool equal(const value& lh, const domain_type& rh)
            {
                switch (lh.attr()) {
                case attributes::MIN_INF:
                case attributes::MAX_INF:
            ...
            static bool equal(const domain_type& lh, const value& rh)
            {
                return equal(rh, lh);
            }
        };
    ...
    inline bool operator<(const value<D, C>& lh, const value<D, C>& rh)
    {
    ...
    template <typename D, typename C>
    inline bool operator<(const value<D, C>& lh, const D& rh)
    {
        using cmp = typename value<D, C>::cmp;
        return cmp::less(lh, rh);
    }
    ...
    template <typename D, typename C>
    inline bool operator<(const D& lh, const value<D, C>& rh)
    {
        using cmp = typename value<D, C>::cmp;
        return cmp::less(lh, rh);
    }
    ...
    template <typename D, typename C>
    inline std::ostream& operator<<(std::ostream& o, const value<D, C>& v)
    {
        switch (v.attr()) {
        case attributes::MAX_INF:
            o << "inf";

...
std::size_t lower_bound(const T* arr, std::size_t length, const T& val,
                        Less less)
{
    std::size_t next = 0;
    while (next < length) {
...
template <typename T, typename Less = std::less<T>>
std::size_t upper_bound(const T* arr, std::size_t length, const T& val,
                        Less less)
{
    size_t next = 0;
    while (next < length) {
...
struct btree {
    ...
    struct cmp {
        ...
        static bool eq(const value_type& l, const value_type& r)
        {
    ...
    struct bnode {
        ...
        void erase(const value_type& val)
        {
            ...
            if (node.first) {
                ...
                if (n->is_leaf()) {
                    ...
                    n->next_[p + 1]->values_.erase(
                        n->next_[p + 1]->values_.begin());
        ...
        void insert(value_type val)
        {
            if (values_.empty()) {
                ...
                auto pos = lower_bound(&values_[0], values_.size(), val, cmp());
                ...
                if (is_leaf()) {
                    ...
                    values_.insert(values_.begin() + pos, std::move(val));
                    ...
                    if (next_[pos]->full()) {
                        ...
                        values_.emplace(values_.begin() + pos, std::move(val));
                    }
                }
            }
        }
        ...
        static std::pair<ptr_type, ptr_type> split(ptr_type src)
        {
    ...
    void insert(value_type val)
    {
        ...
        if (root_->full()) {
            ...
            new_root->values_.insert(new_root->values_.begin(), std::move(val));

    ...
    struct blockchain_varint {
        ...
        size_t len_by_prefix( std::uint8_t prefix )
        {
            ...
            case PREFIX_VARINT64:
                return sizeof(std::uint64_t) + sizeof(std::uint8_t);
        ...
        size_t pack( size_type size, void *result )
        {
            ...
            if( size < PREFIX_VARINT_MIN ) {
                ...
                return u16_little::write( size, &res[1] ) +
                        sizeof(std::uint8_t);
            } else if( size <= 0xFFFFFFFF ) {
                res[0] = static_cast<std::uint8_t>(PREFIX_VARINT32);
                return u32_little::write( size, &res[1] ) +
                        sizeof(std::uint8_t);
            } else {
                res[0] = static_cast<std::uint8_t>(PREFIX_VARINT64);
                return u64_little::write( size, &res[1] ) +
                        sizeof(std::uint8_t);
            }
        }
        ...
        size_t unpack( const void *data, size_t length, size_type *res )
        {
            ...
            if( len == sizeof(std::uint8_t) ) {
                ...
                switch (*d) {
                case PREFIX_VARINT16:
                    res_ = u16_little::read( d + sizeof(std::uint8_t) );
                    break;
                case PREFIX_VARINT32:
                    res_ = u32_little::read( d + sizeof(std::uint8_t) );
                    break;
                case PREFIX_VARINT64:
                    res_ = u64_little::read( d + sizeof(std::uint8_t) );
                    break;
                default:
                    break;
                }
